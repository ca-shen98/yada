import {
  ...
  convertStrValueOrDefaultIfFalsy,
} from "../util/ConvertStrValueOrDefault";
...
export const doSetLocalStorageSourceViews = (sourceId, sourceViews) =>
  localStorage.setItem(
    SOURCE_VIEWS_LOCAL_STORAGE_KEY_PREFIX + sourceId,
    JSON.stringify(sourceViews)
  );

const doGetLocalStorageSourceViews = (sourceId) =>
  convertStrValueOrDefaultIfFalsy(
...
export const doSetLocalStorageSourceIdNames = (sourceIdNames) =>
  localStorage.setItem(
    SOURCE_ID_NAMES_LOCAL_STORAGE_KEY,
    JSON.stringify(sourceIdNames)
  );

const doGetLocalStorageSourceIdNames = () =>
  convertStrValueOrDefaultIfFalsy(
...
export const calculateLocalStorageNextNewId = (existingIdsDict, candidate) => {
  while (!candidate || existingIdsDict.hasOwnProperty(candidate.toString())) {
    candidate += 1;
  }
...
export const calculateLocalStorageNextNewFileIds = (filesList) => ({
  ...
  nextNewViewIdsForSourceIds: Object.fromEntries(
    Object.entries(filesList).map(([sourceId, { views }]) => [
      sourceId,
      calculateLocalStorageNextNewId(views, 0),
    ])
  ),
});
...
const SOURCE_SAVED_TAG_FILTERS_LOCAL_STORAGE_KEY_PREFIX =
  "sourceSavedTagFilters_";
...
export default {
  ...
  doSaveViewSpec: async (tagsList, sourceId, viewId, type, createNew) => {
    const viewSpecLocalStorageKey =
      VIEW_SPEC_LOCAL_STORAGE_KEY_PREFIX + getFileIdKeyStr(sourceId, viewId);
    if (tagsList.length > 0) {
      localStorage.setItem(viewSpecLocalStorageKey, JSON.stringify(tagsList));
    } else {
      localStorage.removeItem(viewSpecLocalStorageKey);
    }
    return {
      id: viewId,
      sourceId,
      ...(createNew ? { name: DEFAULT_VIEW_NAME, type } : null),
    };
  },
  doGetView: async (fileId) =>
    convertStrValueOrDefaultIfFalsy(
      localStorage.getItem(
        VIEW_SPEC_LOCAL_STORAGE_KEY_PREFIX +
          getFileIdKeyStr(fileId.sourceId, fileId.viewId)
      ),
      [],
      "invalid viewSpec"
    ),
  doGetSourceTaggedBlocks: async (sourceId) => {
    const sourceContentDocStr = localStorage.getItem(
      SOURCE_CONTENT_LOCAL_STORAGE_KEY_PREFIX + sourceId
    );
    if (!sourceContentDocStr) {
      return {};
    }
    const sourceContentDoc = convertStrValueOrDefault(
      sourceContentDocStr,
      null,
    ...
    if (sourceContentDoc) {
      ...
      while (nodes.length > 0) {
        ...
        if (node.hasOwnProperty("attrs") && node.attrs.hasOwnProperty("tags")) {
          for (const [tag, tagId] of Object.entries(node.attrs.tags)) {
            sourceTaggedBlocks[tagId] = { tag, node };
          }
  ...
  doSaveSourceContent: async (value, sourceId, createNew) => {
    ...
    const sourceContentLocalStorageKey =
      SOURCE_CONTENT_LOCAL_STORAGE_KEY_PREFIX + sourceId;
    if (saveValue) {
      localStorage.setItem(sourceContentLocalStorageKey, saveValue);
    } else {
      localStorage.removeItem(sourceContentLocalStorageKey);
    }
    return {
      id: sourceId,
      ...(createNew ? { name: DEFAULT_SOURCE_NAME } : null),
    };
  },
