...
class TagMenu extends React.Component {
  ...
  handleAddTag = () => {
    ...
    if (!tag || INVALID_TAG_VALUE_REGEX.test(tag) || !this.props.selectNode) {
      return false;
    }
    if (
    ...
    const newSelectNodeAttrs = {
      ...this.state.selectNodeAttrs,
      tags: { ...this.state.selectNodeAttrs.tags },
    };
  ...
  handleModifyTagValue = () => {
    ...
    if (
      !newTagValue ||
      INVALID_TAG_VALUE_REGEX.test(newTagValue) ||
      !this.props.selectNode ||
      !this.state.selectNodeAttrs.hasOwnProperty("tags") ||
      !this.state.selectNodeAttrs.tags.hasOwnProperty(this.state.currentTag)
    ) {
    ...
    if (newTagValue === this.state.currentTag) {
      return true;
    }
    ...
    newSelectNodeAttrs.tags[newTagValue] =
      newSelectNodeAttrs.tags[this.state.currentTag];
    delete newSelectNodeAttrs.tags[this.state.currentTag];
    BlockTaggingEditorExtension.editor.view.dispatch(
      BlockTaggingEditorExtension.editor.view.state.tr.setNodeMarkup(
        this.props.selectNode.pos,
  ...
  componentDidUpdate = (prevProps) => {
    if (
      ...
      prevProps.currentOpenFileId.viewId !==
        this.props.currentOpenFileId.viewId ||
      (!prevProps.selectNode && this.props.selectNode) ||
      (prevProps.selectNode && !this.props.selectNode) ||
      (prevProps.selectNode &&
        this.props.selectNode &&
      ...
      defer(() => {
        if (this.props.selectNode) {
          ...
          this.setState({
            selectNodeAttrs: {
              ...(node && node.hasOwnProperty("attrs") ? node.attrs : null),
            },
          });
        } else {
        ...
        <div id="tag_menu_list_container">
          ...
          Object.keys(this.state.selectNodeAttrs.tags).length > 0 ? (
            ...
            <div className="PlaceholderDivWithText" id="no_tags_placeholder">
              {"No " +
                (!this.props.selectNode ? "Selected Block" : "Block Tags")}
            </div>
          )}
          <Menu
            id="edit_menu"
          ...
          >
            <Input
              ...
              onKeyDown={(event) => {
                if (event.key === "Escape") {
                  event.target.value = this.state.currentTag;
                }
              }}
              onKeyPress={(event) => {
                if (event.key === "Enter") {
