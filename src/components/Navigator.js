...
export const handleSetCurrentOpenFileId = (
  ...
  if (
    ...
    fileId.viewId === currentOpenFileId.viewId
  ) {
    return true;
  }
...
const countNumFiles = (filesList) =>
  Object.entries(filesList).reduce(
    (count, [_sourceId, { views }]) => count + 1 + Object.keys(views).length,
    0
  );
...
const RENAME_SOURCE_LIST_ITEM_INPUT_ID_PREFIX =
  "rename_source_list_item_input_";
...
const getRenameInputIdFunctions = {
  ...
  [RENAME_INPUT_TYPES.SOURCE_LIST_ITEM]: (fileId) =>
    RENAME_SOURCE_LIST_ITEM_INPUT_ID_PREFIX + fileId.sourceId,
  [RENAME_INPUT_TYPES.VIEW_LIST_ITEM]: (fileId) =>
    RENAME_VIEW_LIST_ITEM_INPUT_ID_PREFIX + getFileIdKeyStr(fileId),
};
      ...
      return (
        <div
          ...
          style={
            this.props.selected
              ? {
                  backgroundColor: "rgba(30, 61, 89, 0.3)",
                  width: "100%",
                  display: "flex",
                  padding: "2px 4px",
                  alignItems: "center",
...
class Navigator extends React.Component {
  ...
  handleSwitchBackendModeSignedInStatus = (status) => {
    if (status === this.props.backendModeSignedInStatus) {
      return true;
    }
  ...
  handleCancelRenaming = (inputType, fileId) => {
    const input = document.getElementById(
      getRenameInputIdFunctions[inputType](fileId)
    );
  ...
  handleChangeSearchingInput = () => {
    this.setState({
      searching: document
        .getElementById(SEARCH_FILE_NAMES_INPUT_ID)
        .value.trim(),
    });
  };

  handleDoFileNamesSearch = () =>
    Object.fromEntries(
      Object.entries(this.state.filesList)
        .map(([sourceId, { name: sourceName, views }]) => [
          sourceId,
          {
            name: sourceName,
            views: this.state.searching
              ? Object.fromEntries(
                  Object.entries(
                    views
                  ).filter(([_viewId, { name: viewName }]) =>
                    viewName.includes(this.state.searching)
                  )
                )
              : views,
          },
        ...
        .filter(
          ([_sourceId, { name: sourceName, views }]) =>
            !this.state.searching ||
            Object.keys(views).length > 0 ||
            sourceName.includes(this.state.searching)
        )
    );

  handleCreateNewFile = async (fileType) => {
    if (!FILE_TYPE.hasOwnProperty(fileType)) {
      return;
    }
    ...
    try {
      ...
      const updatedSourceId = sourceFileType
        ? newFile.id
      ...
      const newFilesList = {
        ...
        [updatedSourceId]: {
          name: sourceFileType
            ? newFile.name
            : this.state.filesList[updatedSourceId].name,
          views: {
            ...(!sourceFileType
              ? {
      ...
      defer(() => {
        const fileId = {
          ...
          viewId: !sourceFileType ? newFile.id : 0,
          viewType: sourceFileType ? FILE_TYPE.EMPTY : newFile.type,
        };
        const fileName = {
      ...
      this.setState({
        renamePopoverElement: document.getElementById(
          getFileIdKeyStr({
            sourceId: updatedSourceId,
            viewId: !sourceFileType ? newFile.id : 0,
            viewType: sourceFileType ? FILE_TYPE.EMPTY : newFile.type,
      ...
      if (
        ...
        if (sourceFileType) {
          doSetLocalStorageSourceIdNames(
            convertFilesListStateToFileIdNamesList(newFilesList)
          );
        } else {
          doSetLocalStorageSourceViews(
            updatedSourceId,
            newFilesList[updatedSourceId].views
          );
        }
        this.setState({
          nextNewFileIds: {
            ...
            nextNewViewIdsForSourceIds: {
              ...
              ...(!sourceFileType
                ? {
                    [updatedSourceId]: calculateLocalStorageNextNewId(
                      newFilesList[updatedSourceId].views,
                      parseInt(newFile.id)
                    ),
                  }
  ...
  handleDeleteFile = async (fileId) => {
    ...
    if (sourceFileIdCheck) {
      ...
      newFilesList[fileId.sourceId] = {
        name: newFilesList[fileId.sourceId].name,
        views: newSourceViews,
      };
    }
    ...
    if (
      ...
      if (sourceFileIdCheck) {
        ...
        const newNextNewViewIdsForSourceIds = {
          ...this.state.nextNewFileIds.nextNewViewIdsForSourceIds,
        };
  ...
  handleRenameFile = async (fileId) => {
    ...
    if (sourceFileIdCheck) {
      newFilesList[fileId.sourceId] = {
        name: newName,
        views: newFilesList[fileId.sourceId].views,
      };
    } else {
      const newSourceViews = {
        ...newFilesList[fileId.sourceId].views,
        [fileId.viewId]: {
          ...newFilesList[fileId.sourceId].views[fileId.viewId],
          name: newName,
        },
      };
      newFilesList[fileId.sourceId] = {
  ...
  componentDidMount = () => {
    ...
    FileStorageSystemClient.doGetFilesList().then((filesList) => {
      ...
      if (
        ...
        this.setState({
          nextNewFileIds: calculateLocalStorageNextNewFileIds(
            this.state.filesList
          ),
        });
      }
  ...
  componentDidUpdate = (prevProps, prevState) => {
    if (
      prevProps.backendModeSignedInStatus !==
      this.props.backendModeSignedInStatus
    ) {
      FileStorageSystemClient.doGetFilesList()
        .then((filesListValue) => filesListValue ?? {})
        .then((filesList) => {
          this.setState({
            ...DEFAULT_STATE,
            filesList,
  ...
  handleFileListClick = (fileId) => {
    if (fileId === this.state.selectedFileId) {
      handleSetCurrentOpenFileId(
        { sourceId: fileId, viewId: 0, viewType: FILE_TYPE.EMPTY },
        { sourceName: this.state.filesList[fileId].name, viewName: "" }
      );
      this.setState({
        selectedViewId: null,
  ...
  handleDeleteMenuClick = () => {
    ...
    const fileId = {
      ...
      viewId:
        this.state.selectedViewId === null ? 0 : this.state.selectedViewId,
    };
      ...
      <div className="SidePane">
        ...
        <div id="file_list_container">
          ...
          {Object.keys(filteredFilesList).length > 0 ? (
            ...
            >
              {Object.entries(filteredFilesList).map(
                ([sourceId, { views }]) => (
                  <div>
                    <ListItem
                      button
